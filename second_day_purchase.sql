...
with all_data as(
select *,dense_rank() over(partition by user_id order by created_at) as rn from marketing_campaign where user_id in(11,14,25)
),
first_day as(
select * from all_data where rn=1
),
all_first_day as(
select * from all_data where rn>1
)
select al.user_id  from all_first_day al 
left join first_day  fd on al.user_id = fd.user_id  and al.product_id = fd.product_id where fd.product_id  is null
